...
class VolumeWindowLevel:
    ...
    @classmethod
    def get_volume_auto_min_max_range(
        cls, volume_node: vtkMRMLVolumeNode
    ) -> tuple[float, float]:
    ...
    @classmethod
    def get_volume_scalar_range(
        cls, volume_node: vtkMRMLVolumeNode
    ) -> tuple[float, float]:
        return volume_node.GetImageData().GetScalarRange()

    @classmethod
    def set_volume_node_window_level(
        cls, volume_node: vtkMRMLVolumeNode, window: float, level: float
    ) -> None:
    ...
    def set_volume_node_display_min_max_range(
        ...
        cls.set_volume_node_window_level(
            volume_node, *cls.min_max_to_window_level(min_value, max_value)
        )

    @classmethod
    def get_volume_display_range(
        cls, volume_node: vtkMRMLVolumeNode
    ) -> tuple[float, float]:
        d_node = cls.get_volume_display_node(volume_node)
    ...
    @classmethod
    def window_level_to_min_max(
        cls, window: float, level: float
    ) -> tuple[float, float]:
        return level - window / 2, level + window / 2

    @classmethod
    def min_max_to_window_level(
