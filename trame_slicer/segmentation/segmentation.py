...
class SegmentationLabelMapUndoCommand(UndoCommand):
    ...
    def undo(self) -> None:
        if self._segmentation.editor_logic:
            self._segmentation.editor_logic.Undo()

    def redo(self) -> None:
        if self._segmentation.editor_logic:
            self._segmentation.editor_logic.Redo()

    @classmethod
    @contextmanager
    def push_state_change(cls, segmentation: Segmentation):
        if not segmentation.editor_logic or not segmentation.undo_stack:
            yield
            return

        segmentation.editor_logic.SaveStateForUndo()
        yield
        segmentation.undo_stack.push(cls(segmentation))
