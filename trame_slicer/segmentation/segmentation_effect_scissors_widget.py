...
class SegmentScissorsWidget:
    ...
    def set_segment_modifier(self, modifier: SegmentModifier):
    ...
    def _display_to_world_generic(
        ...
        renderer = self._view_node.renderer()
...
class SegmentScissorPipeline(vtkMRMLLayerDMScriptedPipeline):
    def __init__(self) -> None:
        super().__init__()

        self._widget = SegmentScissorsWidget()
    ...
    def CanProcessInteractionEvent(self, eventData: vtkMRMLInteractionEventData) -> tuple[bool, float]:
        if not self.is_active:
            return super().CanProcessInteractionEvent(eventData)
        return True, 0.0
    ...
    @property
    def widget(self) -> SegmentScissorsWidget:
        return self._widget
