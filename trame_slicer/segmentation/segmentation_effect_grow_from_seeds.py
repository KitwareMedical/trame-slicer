...
from slicer import vtkMRMLAbstractViewNode, vtkMRMLNode, vtkMRMLSegmentationNode, vtkOrientedImageData

try:
    import vtkITK
except ModuleNotFoundError:
    import vtkmodules.vtkITK as vtkITK
...
class SegmentationEffectGrowFromSeeds(SegmentationEffectPaintErase):
    ...
    def _copy_preview_segmentation_to_segmentation(self) -> None:
        ...
        try:
            self._modifier.apply_labelmap(self._preview_modifier.get_merged_label_map())
        finally:
    ...
    def _update_segmentation_observer(self) -> None:
        if self.is_active:
            self._modifier.segmentation_modified.connect(self._on_segmentation_modified)
        else:
            self._modifier.segmentation_modified.disconnect(self._on_segmentation_modified)
